...
for obs_name, selected in list(actions_.values())[0]['states'].items():
    ...
    if obs_name == 'hour':
        break
    assert indx_hour < len(list(actions_.values())[0][
                               'states'].items()) - 1, "Please, select hour as a state for Building_1 to run the RBC"
...
# _, next_state, rewards, done, _ = env.step(action)
