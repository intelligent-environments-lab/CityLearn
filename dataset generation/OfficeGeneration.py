...
CHANGE_ROUTINE_WEEK = 0.03
CHANGE_ROUTINE_WEEKEND = 0.3
WORK_WEEKEND_CONSTANT = 0.10
WORK_WEEKEND_RAND_1 = 0.05
...
semana_dia = [ # Prob ; Min Left ; Max Left 
    ("Normal1", 0.40,7,8), # 7-8  
    ("Normal2", 0.50,8,9), # 8-9  
    ("Cedo", 0.05,5,6), # 5-6  
    ("tarde", 0.05,9,14), # 9-12   
]
...
def day_gen(arrive_hour,leave_hour,trip_time,dict_charge,month,day_type,arrive_soc):
    ...
    if(want_to_charge and charger!=None): # Yes i have a charge
        leave_hour = min(info_charge["time"]+arrive_hour,leave_hour)
        ...
        leave_hour = min(arrive_hour_charger+info_charge["time"],leave_hour) # Lets see if leave hour is greater than the after charge
...
# lets fill the files
def to_file():
    with open("C:\\Users\\steam\\Desktop\\Opeva\\"+'ev_id_10.csv', 'w') as file:
    ...
    with open("C:\\Users\\steam\\Desktop\\Opeva\\"+'ev_id_9.csv', 'w') as file:
        header = "Month,Hour,Day Type,State,Destination Charger,Estimated Departure Time,Required Soc At Departure, Estimated Arrival Time, Estimated SOC at Arrival"
        file.write(header+"\n")
        for x in store["ev_id_9"]:
            file.write(x)
            
    with open("C:\\Users\\steam\\Desktop\\Opeva\\"+'ev_id_8.csv', 'w') as file:
        header = "Month,Hour,Day Type,State,Destination Charger,Estimated Departure Time,Required Soc At Departure, Estimated Arrival Time, Estimated SOC at Arrival"
        file.write(header+"\n")
        for x in store["ev_id_8"]:
            file.write(x)

    with open("C:\\Users\\steam\\Desktop\\Opeva\\"+'ev_id_7.csv', 'w') as file:
        header = "Month,Hour,Day Type,State,Destination Charger,Estimated Departure Time,Required Soc At Departure, Estimated Arrival Time, Estimated SOC at Arrival"
        file.write(header+"\n")
