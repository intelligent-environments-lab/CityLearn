...
class SAC:
    def __init__(
        ...
        **kwargs
        ...
        self.action_space = {uid : a_space for uid, a_space in zip(building_ids, action_space)}
        self.observation_space = {uid : o_space for uid, o_space in zip(building_ids, observation_space)}
        self.encoders = {uid:e for uid, e in zip(building_ids, encoders)}
        ...
        self.replay_buffer, self.soft_q_net1, self.soft_q_net2, self.target_soft_q_net1, self.target_soft_q_net2, self.policy_net, self.soft_q_optimizer1, self.soft_q_optimizer2, self.policy_optimizer, self.target_entropy, self.alpha, self.norm_mean, self.norm_std, self.r_norm_mean, self.r_norm_std, self.norm_mean, self.norm_std, self.r_norm_mean, self.r_norm_std, = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
        ...
        for uid in building_ids:
            ...
            #     if not s:
            #         self.encoders[uid].append(0)
            #     elif s_name in ["month", "hour"]:
            #         self.encoders[uid].append(PeriodicNormalization(self.observation_space[uid].high[state_n]))
            #         state_n += 1
            #     elif s_name == "day":
            #         self.encoders[uid].append(OnehotEncoding([1,2,3,4,5,6,7,8]))
            #         state_n += 1
            #     elif s_name == "daylight_savings_status":
            #         self.encoders[uid].append(OnehotEncoding([0,1]))
            #         state_n += 1
            #     elif s_name == "net_electricity_consumption":
            #         self.encoders[uid].append(RemoveFeature())
            #         state_n += 1
            #     else:
            #         self.encoders[uid].append(Normalize(self.observation_space[uid].low[state_n], self.observation_space[uid].high[state_n]))
            #         state_n += 1  

            # self.encoders[uid] = np.array(self.encoders[uid])

            # # If there is no solar PV installed, remove solar radiation variables 
            # if building_info[uid]['solar_power_capacity (kW)'] == 0:
            #     for k in range(12,20):
            #         if self.encoders[uid][k] != 0:
            #             self.encoders[uid][k] = -1
            #     if self.encoders[uid][24] != 0:
            #         self.encoders[uid][24] = -1
            # if building_info[uid]['Annual_DHW_demand (kWh)'] == 0 and self.encoders[uid][26] != 0:
            #     self.encoders[uid][26] = -1
            # if building_info[uid]['Annual_cooling_demand (kWh)'] == 0 and self.encoders[uid][25] != 0:
            #     self.encoders[uid][25] = -1
            # if building_info[uid]['Annual_nonshiftable_electrical_demand (kWh)'] == 0 and self.encoders[uid][23] != 0:
            #     self.encoders[uid][23] = -1

            # self.encoders[uid] = self.encoders[uid][self.encoders[uid]!=0]
            # self.encoders[uid][self.encoders[uid]==-1] = RemoveFeature()
            
            state_dim = len([j for j in np.hstack(self.encoders[uid]*np.ones(len(self.observation_space[uid].low))) if j != None])
                
            action_dim = self.action_space[uid].shape[0]
            self.alpha[uid] = 0.2
            
            self.replay_buffer[uid] = ReplayBuffer(int(replay_buffer_capacity))
            
            # init networks
            self.soft_q_net1[uid] = SoftQNetwork(state_dim, action_dim, hidden_dimension).to(self.device)
            self.soft_q_net2[uid] = SoftQNetwork(state_dim, action_dim, hidden_dimension).to(self.device)

            self.target_soft_q_net1[uid] = SoftQNetwork(state_dim, action_dim, hidden_dimension).to(self.device)
            self.target_soft_q_net2[uid] = SoftQNetwork(state_dim, action_dim, hidden_dimension).to(self.device)
            ...
            # Policy
            self.policy_net[uid] = PolicyNetwork(state_dim, action_dim, self.action_space[uid], self.action_scaling_coef, hidden_dimension).to(self.device)
            ...
            self.target_entropy[uid] = -np.prod(self.action_space[uid].shape).item()
                  
    def select_action(self, states):
        states = np.array(states)
        self.time_step += 1
        explore = self.time_step <= self.end_exploration_time_step
        actions = []
        k = 0
        ...
        for uid, state in zip(self.building_ids, states):
            if explore:
                actions.append(self.action_scaling_coef*self.action_space[uid].sample())
            else:
                state_ = np.array([j for j in np.hstack(self.encoders[uid]*state) if j != None])
    ...
    def add_to_buffer(self, states, actions, rewards, next_states, done, coordination_vars, coordination_vars_next):
        ...
        for (uid, o, a, r, o2,) in zip(self.building_ids, states, actions, rewards, next_states):            
            ...
            o = np.array([j for j in np.hstack(self.encoders[uid]*o) if j != None])
            o2 = np.array([j for j in np.hstack(self.encoders[uid]*o2) if j != None])
        ...
        if self.time_step >= self.start_training_time_step and self.batch_size <= len(self.replay_buffer[self.building_ids[0]]): 
